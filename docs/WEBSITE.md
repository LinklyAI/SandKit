...
样式依据官网 `app/themes/cool.css` 与 `HeroSection.tsx`：暖白 `#fffbf9`、中性黑灰、琥珀色 `#f59e0b`、无衬线轻字重标题。保留官网配色，采用简洁的技术项目布局、直角菜单与按钮；沙画直接融入页面，不使用卡片或画布边框。沙画保留蓝色，默认文字演示为放大的 `LINKLY AI`。
